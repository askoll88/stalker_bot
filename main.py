...
def create_main_keyboard(shelter_unlocked=False):
    ...
    keyboard.add_button("Attack", color=VkKeyboardColor.NEGATIVE)
    keyboard.add_line()
...
def create_shelter_keyboard():
    keyboard = VkKeyboard(one_time=False)
    keyboard.add_button("Rest", color=VkKeyboardColor.POSITIVE)
    keyboard.add_line()
    keyboard.add_button("To City", color=VkKeyboardColor.NEGATIVE)
    return keyboard
...
def handle_player_action(vk, user_id, action):
    ...
    if player['health'] <= 0:
        send_message(vk, user_id, "You died! Go to hospital.",
...
def handle_heal(vk, user_id, player):
    ...
    if player['money'] >= HEAL_COST:
        ...
        send_message(vk, user_id, f"You healed! +50 health. Spent: {HEAL_COST} rub",
                    keyboard=create_hospital_keyboard().get_keyboard())
    else:
...
def handle_rest(vk, user_id, player):
    if player['current_location'] != "shelter":
        ...
        return
    if player['fatigue'] == 0:
        send_message(vk, user_id, "You are full of energy!",
    ...
    send_message(vk, user_id, f"You rested! Fatigue: {player['fatigue']} -> {new_fatigue}",
                keyboard=create_shelter_keyboard().get_keyboard())
...
def handle_attack(vk, user_id, player):
    ...
    if monster['health'] <= 0:
        ...
        msg = f"FIGHT WITH {monster['name']}\n\nYou dealt {player_damage} damage!\nEnemy dealt {monster_damage} damage!\n\nVICTORY! +{exp_gain} exp +{money_gain} rub"
        if level_up:
...
def handle_buy_item(vk, user_id, player, item_id, price):
    ...
    if success:
        ...
        send_message(vk, user_id, f"Bought: {item['name']}. Spent: {price} rub",
                    keyboard=create_market_keyboard().get_keyboard())
    else:
...
def move_to_location(vk, user_id, location_id):
    ...
    if location_id == "shelter" and not player['shelter_unlocked']:
        send_message(vk, user_id, "Shelter is locked.")
        return
    ...
    if player['fatigue'] >= MAX_FATIGUE:
        send_message(vk, user_id, "You are too tired!",
                    keyboard=create_main_keyboard(player['shelter_unlocked']).get_keyboard())
        return
        ...
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            if text == '/start' or text == 'start' or text == 'begin':
                if not db.player_exists(user_id):
                    ...
                    welcome_msg = "WELCOME TO THE ZONE, STALKER!\n\nYou arrived at an abandoned city. Your adventure starts here.\n\nYou are in the City.\nShelter is locked - find a way to get inside.\n\nUse buttons for navigation!"
                    ...
                    vk.messages.send(user_id=user_id,
                                    message=f"Welcome back, stalker {player['name']}! You are in: {get_location(player['current_location']).name}",
                                    keyboard=create_main_keyboard(player['shelter_unlocked']).get_keyboard(),
                                    random_id=random.randint(0, 2**31))
